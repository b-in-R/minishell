...
void	only_builtin(t_cmd *cmd, t_expander *exp, t_fork *data)
{
	...
	if (save_in == -1 || save_out == -1 || save_err == -1)
		error_exit(exp->pool, exp->my_env,
			"only_builtin: dup save std fds");
	if (!pool_track_fd(exp->pool, save_in))
		error_exit(exp->pool, exp->my_env,
			"only_builtin: track save_in");
	if (!pool_track_fd(exp->pool, save_out))
		error_exit(exp->pool, exp->my_env,
			"only_builtin: track save_out");
	if (!pool_track_fd(exp->pool, save_err))
		error_exit(exp->pool, exp->my_env,
			"only_builtin: track save_err");
	data->fd[0] = -1;
	data->fd[1] = -1;
	...
	if (dup2(save_in, STDIN_FILENO) == -1
		...
		error_exit(exp->pool, exp->my_env,
			"only_builtin: dup2 restore std fds");
	pool_close_ctx(exp->pool, save_in);
	pool_close_ctx(exp->pool, save_out);
	pool_close_ctx(exp->pool, save_err);
...
void	execute_bis_2(t_cmd **cmd, t_expander *exp, t_fork *data)
{
	...
	if (data->fd[0] != -1)
		pool_close_ctx(exp->pool, data->fd[0]);
	if (data->fd[1] != -1)
		pool_close_ctx(exp->pool, data->fd[1]);
	if (data->in_fd != 0)
		pool_close_ctx(exp->pool, data->in_fd);
	if (!(*cmd)->args || !(*cmd)->args[0])
	{
...
void	execute(t_cmd *cmd, t_expander *exp)
{
	...
	if (!cmd->next && is_builtin(cmd))
	{
		...
		pool_free_ctx(exp->pool, data.pid);
		return ;
	}
