...
{
	if (cmd->infile)
		pool_free_ctx(exp->pool, cmd->infile);
	cmd->infile = pool_strdup_ctx(exp->pool, value);
}

void	handle_redir_out(t_expander *exp, t_cmd *cmd, char *value, int type)
{
	if (cmd->outfile)
		pool_free_ctx(exp->pool, cmd->outfile);
	cmd->outfile = pool_strdup_ctx(exp->pool, value);
...
{
	if (cmd->delimiter)
		pool_free_ctx(exp->pool, cmd->delimiter);
	cmd->heredoc = 1;
